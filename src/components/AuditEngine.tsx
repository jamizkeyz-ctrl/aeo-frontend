...
function DashboardContent() {
  ...
  // Static marketing shell lives in the server-rendered page. Hide it while a
  // report or signed-in workspace is on screen, without unmounting server HTML.
  useEffect(() => {
    const active = Boolean(summaryData) || Boolean(user);
    document.body.dataset.workspace = active ? "active" : "idle";
    return () => { document.body.dataset.workspace = "idle"; };
  }, [summaryData, user]);
  ...
  return (
    <>
      ...
      {/* FROSTED NAVBAR */}
        {/* ENGINE / REPORT WORKSPACE */}
        <div className="max-w-7xl mx-auto px-6 pt-6 pb-12 space-y-16">
...
export default function AuditEngine() {
  return (
    <Suspense fallback={<div className="max-w-7xl mx-auto px-6 py-16 flex items-center justify-center text-slate-500"><Loader2 className="h-5 w-5 animate-spin" /></div>}>
      <DashboardContent />
    </Suspense>
  );
}